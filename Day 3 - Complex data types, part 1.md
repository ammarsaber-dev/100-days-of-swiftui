...
**Previous:** [Day 2 - Simple data types, part 2](Day%202%20-%20Simple%20data%20types,%20part%202.md) | **Next:** Day 4 - Complex data types, part 2
...
**Reading by index** (position starts at 0):
...
**Important:** Accessing an index that doesn't exist will crash your app.
...
```swift
beatles.append("Adrian")
beatles.append("Allen")  // can add same item multiple times
```
...
Swift won't let you mix types, even when reading:
...
Two ways to create empty arrays:
...
Swift can infer the type if you provide initial values:
...
var characters = ["Lana", "Pam", "Ray", "Sterling"]
characters.remove(at: 2)  // removes "Ray"
characters.removeAll()     // removes everything
...
// Reverse (clever: doesn't actually rearrange, just remembers)
let presidents = ["Bush", "Obama", "Trump", "Biden"]
let reversedPresidents = presidents.reversed()
```

**Note:** `sorted()`, `reversed()`, and other array methods also work on strings.
...
// WRONG: Mixing types
var mixed = [1, 2, "three"]  // Error

// WRONG: Modifying constant array
let scores = [100, 90, 85]
scores.append(95)  // Error
...
**Key Takeaway:** Arrays store ordered collections of the same type. Index starts at 0. Use `var` for mutable arrays, `let` for fixed arrays. Methods: `append()`, `remove()`, `contains()`, `sorted()`, `count`.

### Optional: Why Does Swift Have Arrays?

Swift's strings, integers, Booleans, and Doubles store single values, but arrays let you store many values together.
...
Arrays can be any size and grow/shrink as needed (if variable). You read values by numerical position starting from 0 (**zero-based indexing**).

Swift crashes your program if you try to read an invalid index (like index 10 in a 3-item array). This seems bad, but it's actually protecting you—better to crash than silently return wrong data.

---

## 2. How to Store and Find Data in Dictionaries

Arrays are great for ordered data, but accessing items by position can be annoying or dangerous. **Dictionaries** store data using meaningful keys instead of positions.
...
- Can't be sure what each position means
- No guarantee an index exists
- Easy to break if items are removed
...
What if you read a key that doesn't exist?

```swift
print(employee2["password"])  // key doesn't exist!
```

Swift doesn't crash like with arrays. Instead, it returns an **optional** – the data might be there or might not. Swift warns you with "Expression implicitly coerced from 'String?' to 'Any'".
...
If the key doesn't exist, Swift uses the default value instead.

### Different Data Types

Dictionaries can use different types for keys and values:
...
Specify types explicitly when creating empty dictionaries:
...
Dictionaries don't allow duplicate keys. Setting a value for an existing key overwrites the old value:
...
Like arrays, dictionaries have `count` and `removeAll()`:
...
var scores = ["Player": 100]
scores["Player"] = 200  // overwrites 100, doesn't add duplicate

// Empty dictionary needs type annotation
var data = [:]  // Error: needs type
var data = [String: Int]()  // Correct
```

**Key Takeaway:** Dictionaries store data with meaningful keys instead of positions. Reading a non-existent key returns an optional, not a crash. Use `default:` to provide fallback values. Keys must be unique – setting an existing key overwrites its value.

### Optional: Why Does Swift Have Dictionaries as Well as Arrays?

Both dictionaries and arrays store lots of data, but differently:

- **Arrays:** Items stored sequentially by index (0, 1, 2...)
- **Dictionaries:** Items stored by meaningful keys you choose

Dictionaries are more convenient – instead of remembering "index 7 is country", just write `user["country"]`.

Dictionaries optimize for fast retrieval. Reading `user["country"]` returns instantly, even with 100,000 items.

**Caveat:** Keys might not exist, so you get back an optional (value or nil).

### Optional: Why Does Swift Have Default Values for Dictionaries?

Reading from a dictionary might return nil if the key doesn't exist. This can cause problems in your code.

Default values provide a backup when a key doesn't exist:
...
// Student didn't take history exam
let historyResult = results["history", default: 0]  // returns 0
```

**When to use defaults:**

- If missing value = failure/zero: use default value (always get a value back)
- If missing value = hasn't happened yet: don't use default (check for nil)

Not all situations need defaults, but when you do, they're easy to use.

---

## 3. How to Use Sets for Fast Data Lookup

**Sets** are like arrays but with two key differences:
...
You create an array first, then put it into the set. The set automatically removes duplicates and doesn't maintain the order.

```swift
print(people)  // order might differ from what you specified
```
...
`insert()` makes sense because sets don't have an "end" – they store items in whatever order they want.

### Why Use Sets?

**1. No duplicates allowed** Perfect for situations where uniqueness matters (like Screen Actors Guild requiring unique stage names).

**2. Extremely fast lookups** Sets are highly optimized for finding items.

Example: Checking if an array contains "The Dark Knight":

- Array of 1,000 items: might check all 1,000 before returning false
- Set of 1,000,000 items: returns instantly

The speed difference is massive. Even with 10 million items, `contains()` on a set runs instantly, while an array might take minutes.

### Useful Set Methods

```swift
people.contains("Tom Cruise")  // extremely fast
people.count                   // number of items
people.sorted()                // returns sorted array
```

**Note:** Most of the time you'll use arrays, but when you need fast lookups or guaranteed uniqueness, sets are the perfect choice.

### Common Mistakes

```swift
// WRONG: Using append() on sets
var names = Set<String>()
names.append("John")  // Error: sets use insert()
...
// CORRECT: Use contains() or iterate
print(set.contains("A"))
```

**Key Takeaway:** Sets store unique, unordered items with extremely fast lookups. Use `insert()` to add items. Perfect for checking "does this exist?" or when duplicates aren't allowed. Use arrays when order matters or duplicates are needed.

### Optional: Why Are Sets Different from Arrays in Swift?

Both sets and arrays hold multiple values, but they differ in two key ways:

- **Sets:** Unordered, no duplicates
- **Arrays:** Ordered, allows duplicates

**Why this matters:** Sets store items in an optimized order for fast retrieval. Checking "does this set contain item X?" returns instantly, regardless of size.

Arrays must check every item in order. With 10,000 items, this could mean checking all 10,000 before returning false.

**When to use sets:**

- Checking if an item exists ("does this word appear in a dictionary?")
- Ensuring uniqueness
- Need fast lookups
...
For more details: https://www.avanderlee.com/swift/array-vs-set-differences-explained/

---

## 4. How to Create and Use Enums

An **enum** (enumeration) is a set of named values you can create and use in your code. They're more efficient and safer than using strings.
...
var selected = "Monday"
selected = "Tuesday"
selected = "January"  // Oops! Wrong - but Swift allows it
selected = "Friday "  // Extra space - different from "Friday"
```

Using strings for this requires careful programming and is inefficient (storing all letters of "Friday" just to track a day).

### Creating Enums

Enums let you define specific values a type can have:
...
Now you can only use these five specific values – no typos, no accidents.
...
Swift won't let you use invalid values like "January" or "Friday " with a space. When you type `Weekday.`, Swift shows all possible options.
...
**2. Skip enum name after first assignment:**
...
Swift remembers `day` is always a `Weekday`, so you can use shorthand.

**Behind the scenes:** Swift stores enums as simple integers (0, 1, 2...), making them much faster than strings.

### Common Mistakes

```swift
// WRONG: Using invalid enum case
var day = Weekday.monday
day = Weekday.saturday  // Error: saturday doesn't exist in Weekday

// WRONG: Mixing strings with enums
var day = Weekday.monday
day = "Tuesday"  // Error: can't assign String to Weekday

// CORRECT: Use defined cases only
var day = Weekday.monday
day = .tuesday

// Note: Enums are type-safe
enum Direction {
    case north, south, east, west
}
var heading = Direction.north
heading = Weekday.monday  // Error: can't mix Direction and Weekday
```

**Key Takeaway:** Enums define a set of valid values for a type. They prevent typos and invalid data. More efficient than strings. Use shorthand notation after first assignment. Swift enforces that only valid cases can be used.

### Optional: Why Does Swift Need Enums?

Enums are a way of giving meaningful names to values.

**Without enums:**

```swift
let direction = 1  // What does 1 mean? North? East?
let invalid = 5    // Nothing prevents this mistake
```
...
let heading = Direction.north  // Clear and safe
let invalid = Direction.thatWay  // Error: doesn't exist
```

**Benefits:**

- Clear, meaningful names (`Direction.north` vs `1`)
- Swift refuses to compile invalid cases
- Stored as simple integers behind the scenes (very fast)
- Much more powerful in Swift than other languages

Enums make code safer, clearer, and faster.
...
- Store ordered collections of the same type
- Access by index (starts at 0)
- Methods: `append()`, `remove(at:)`, `removeAll()`, `contains()`, `sorted()`, `reversed()`, `count`
- Create empty: `var scores = [Int]()` or `var scores = Array<Int>()`
- Type-safe: can only hold one type

**Dictionaries**

- Store key-value pairs with meaningful keys
- Reading a key returns an optional (might not exist)
- Use `default:` to provide fallback values
- Create empty: `var heights = [String: Int]()`
- Keys must be unique – setting existing key overwrites value
- Methods: `count`, `removeAll()`
- Fast retrieval regardless of size

**Sets**

- Store unique, unordered items
- Extremely fast lookups with `contains()`
- Use `insert()` to add items (not `append()`)
- Automatically remove duplicates
- Create: `let people = Set(["Name1", "Name2"])` or `var people = Set<String>()`
- Methods: `contains()`, `count`, `sorted()`
- Perfect for "does this exist?" questions

**Enums**

- Define a set of valid named values
- Prevent typos and invalid data
- More efficient than strings
- Use shorthand after first assignment: `.monday` instead of `Weekday.monday`
- Type-safe: can only use defined cases
- Stored as integers behind the scenes

**Key Principle:** Choose the right collection type:

- Need order and duplicates? → **Array**
- Need meaningful keys? → **Dictionary**
- Need fast lookups and uniqueness? → **Set**
- Need specific valid values? → **Enum**
